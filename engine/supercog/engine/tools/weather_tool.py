...
"""
    ...
    def _get_current_datetime_with_timezone(self):
        ...
        system_timezone = zoneinfo.ZoneInfo(zoneinfo.available_timezones().pop())  # or replace with your system's timezone string
    ...
    def get_current_weather(
        ...
        if response.status_code == 200:
            ...
            if hourly and current_index is not None:
                ...
                apparent_temp = hourly.get('apparent_temperature', [None])[current_index]
            ...
            return LLMFullResult(return_string)
        else:
    ...
    def get_forecast_weather(
        ...
        response = requests.get(url, params=params)
        return_string = f"Current Date and time is: {self._get_current_datetime_with_timezone()}\n"
            ...
            if forecast_type == 'hourly':
                ...
                if hourly:
                    ...
                    for i in range(len(times)):
                        ...
                        if feels_like is not None:
                            return_string += f"  Feels Like: {feels_like}{temp_symbol}\n"
                        ...
                        direction = hourly.get('winddirection_10m', [None]*len(times))[i]
                ...
                if daily:
                    ...
                    for i in range(len(times)):
                        ...
                        prob = daily.get('precipitation_probability_max', [None]*len(times))[i]
                        if prob is not None:
                        ...
                        direction = daily.get('winddirection_10m_dominant', [None]*len(times))[i]
                        if direction is not None:
                            return_string += f"  Dominant Wind Direction: {direction}°\n"
    ...
    def get_historical_weather(
        ...
        Notes:
            ...
            - Maximum range per request is 1 year
            - Data quality and availability varies by location and time period
        """
        ...
        if response.status_code == 200:
            ...
            if daily:
                ...
                for i in range(len(times)):
                    ...
                    wind_dir = daily.get('winddirection_10m_dominant', [None]*len(times))[i]
                    ...
                    evapotranspiration = daily.get('et0_fao_evapotranspiration', [None]*len(times))[i]
            ...
            return f"Failed to retrieve historical data: {response.status_code}\nResponse: {response.text}"
    ...
    def get_historical_averages(
        ...
        try:
            ...
            try:
                ...
                target_start = datetime.strptime(f"{current_year}-{target_start_date}", "%Y-%m-%d")
                target_end = datetime.strptime(f"{current_year}-{target_end_date}", "%Y-%m-%d")
            ...
            # Format the response
            dates = [(target_start + timedelta(days=i)).strftime("%m-%d") for i in range(num_days)]
            temp_symbol = '°F' if temperature_unit.lower() == 'fahrenheit' else '°C'
            ...
            for i, date in enumerate(dates):
                ...
                if averaged_data['snowfall_sum'][i]:
                    formatted_text += f"  Snowfall: {averaged_data['snowfall_sum'][i]}cm\n"
            ...
            return LLMFullResult(formatted_text)
